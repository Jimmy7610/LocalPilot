      ...
      >
        ...
        <motion.div variants={item} className="flex items-end justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-[10px] font-bold uppercase tracking-widest text-white/30">Local-First Agent Core</div>
            </div>
            <h1 className="text-5xl font-bold tracking-tighter leading-none">
                <span className="text-white/40 font-medium italic mr-3">Welcome to</span>
                <span className="text-foreground italic font-serif">LocalPilot</span>
            </h1>
          </div>
          <div className="flex flex-col items-end gap-1">
             <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-sm shadow-xl">
        ...
        <motion.div variants={item}>
           <button 
             ...
             className="w-full group relative flex items-center justify-between p-6 rounded-[28px] bg-white/5 border border-white/5 hover:border-primary/20 hover:bg-white/10 transition-all duration-300 shadow-2xl group shadow-black/40"
           >
              <div className="flex items-center gap-6">
                 ...
                 <div className="text-left">
                    <p className="text-xl font-bold text-white/50 group-hover:text-white transition-colors tracking-tight">Search everything or ask anything...</p>
                    <p className="text-xs font-medium text-white/20 uppercase tracking-widest mt-0.5">Quick access to projects, chats, and intelligent tools</p>
                 </div>
              </div>
              <div className="flex items-center gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                 <kbd className="px-2 py-1 rounded-lg bg-black/40 border border-white/10 font-mono text-xs font-bold">CTRL</kbd>
                 <kbd className="px-2 py-1 rounded-lg bg-black/40 border border-white/10 font-mono text-xs font-bold">K</kbd>
              </div>
           </button>
        </motion.div>
        ...
        <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-8 space-y-8">
                ...
                <motion.div variants={item} className="grid grid-cols-4 gap-4">
                  {[
                    ...
                    { label: t.home.newProject, icon: FolderKanban, action: () => navigate('/projects'), color: 'text-white/60', bg: 'bg-white/5' },
                    { label: t.home.newPrompt, icon: BookOpen, action: () => navigate('/prompts'), color: 'text-white/60', bg: 'bg-white/5' },
                    { label: t.home.newDocument, icon: FileText, action: () => navigate('/documents'), color: 'text-white/60', bg: 'bg-white/5' },
                  ].map((action) => (
                    <button
                      key={action.label}
                      onClick={action.action}
                      className="group flex flex-col items-center justify-center gap-3 p-6 rounded-3xl bg-white/5 border border-white/5 hover:border-primary/20 hover:bg-primary/5 transition-all"
                    >
                ...
                <motion.div variants={item}>
                    <div className="flex items-center justify-between mb-4 px-2">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white/20">{t.home.recentChats}</h3>
                    ...
                    <div className="space-y-3">
                        {recentChats.length === 0 ? (
                            <div className="p-12 rounded-3xl border border-dashed border-white/5 flex flex-col items-center justify-center opacity-40">
                            ...
                            recentChats.map((chat) => (
                                <button
                                    ...
                                    className="group flex items-center justify-between w-full p-4 rounded-3xl bg-white/5 border border-white/5 hover:border-white/10 transition-all text-left shadow-lg shadow-black/20"
                                >
                                    <div className="flex items-center gap-4 min-w-0">
                                        <div className="w-10 h-10 rounded-2xl bg-black/40 flex items-center justify-center shrink-0 border border-white/5 group-hover:bg-primary/20 group-hover:border-primary/40 transition-all">
                                            <MessageSquare className="w-4 h-4 text-white/20 group-hover:text-primary transition-colors" />
                                        </div>
                                        <div className="min-w-0">
                                            <p className="text-sm font-bold truncate group-hover:text-white transition-colors">{chat.title}</p>
                                            <p className="text-[10px] font-bold uppercase tracking-widest text-white/20 mt-0.5">
                                    ...
                                    <div className="flex items-center gap-4 px-4 opacity-0 group-hover:opacity-100 transition-all">
                                        <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded bg-white/5 text-white/40">{chat.model}</span>
            ...
            <div className="col-span-4 space-y-8">
                ...
                <motion.div variants={item} className="p-8 rounded-[32px] bg-primary/10 border border-primary/20 shadow-2xl shadow-primary/5 space-y-8">
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                             <Wifi className={cn("w-4 h-4", connected ? "text-primary animate-pulse" : "text-white/20")} />
                ...
                {/* Recent Documents */}
                <motion.div variants={item}>
                    <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white/20 mb-4 px-2">{t.home.documentsSnapshot}</h3>
                    <div className="p-3 rounded-[32px] bg-white/5 border border-white/5 space-y-1 shadow-xl shadow-black/20">
                        {recentDocs.length === 0 ? (
                            ...
                            recentDocs.map(doc => (
                                <button
                                    ...
                                    className="group flex items-center gap-3 w-full p-3 rounded-2xl hover:bg-white/5 transition-all text-left"
                                >
                                    <div className="w-8 h-8 rounded-xl bg-black/40 flex items-center justify-center border border-white/5 opacity-40 group-hover:opacity-100 group-hover:text-primary transition-all">
                                        <FileText className="w-3.5 h-3.5" />
                                    </div>
                                    <div className="min-w-0">
                                        <p className="text-xs font-bold truncate opacity-60 group-hover:opacity-100 group-hover:text-white transition-all">{doc.title}</p>

      ...
      <div className="max-w-6xl mx-auto p-12 space-y-12 animate-in fade-in duration-700">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-lg text-foreground/40 font-medium italic">High-performance AI instructions.</p>
          </div>
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
                <div className="relative flex-1 w-full group">
                    ...
                    <Input
                        ...
                        className="pl-12 h-14 rounded-[20px] glass border-border focus:border-primary/40 transition-all font-medium italic text-lg"
                    />
                </div>
                <Button
                    variant="ghost"
                    className={cn(
                        'h-14 px-8 rounded-[20px] glass border-border font-black uppercase tracking-widest text-[10px] gap-3 transition-all',
                        showFavorites ? 'bg-primary/20 text-primary border-primary/20' : 'text-foreground/40 hover:text-foreground'
                    )}
            ...
            <Tabs value={activeCategory} onValueChange={setActiveCategory} className="w-full">
                <TabsList className="h-12 p-1 bg-muted/50 border border-border rounded-2xl backdrop-blur-xl">
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-center glass-card border-border rounded-[40px] opacity-40 group hover:opacity-60 transition-opacity">
            <div className="w-24 h-24 rounded-full bg-muted flex items-center justify-center mb-6 shadow-xl border border-border group-hover:scale-110 transition-transform">
            ...
            <p className="text-foreground/40 max-w-sm mb-10">{t.prompts.noPromptsHint}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filtered.map(prompt => (
              ...
              >
                ...
                <div className="glass-card border-border rounded-[32px] p-8 hover:border-primary/20 transition-all shadow-xl group-hover:shadow-2xl h-full flex flex-col isolation-isolate overflow-hidden bg-muted/20">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex-1 min-w-0 pr-4">
                        ...
                        <p className="text-xs text-foreground/40 font-medium line-clamp-1 italic mt-1">{prompt.description}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                        <Button
                            variant="ghost"
                            size="icon"
                            className="h-9 w-9 text-foreground/20 hover:text-foreground rounded-xl hover:bg-muted/50"
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-9 w-9 text-foreground/20 hover:text-foreground rounded-xl hover:bg-muted/50">
                  ...
                  <div className="flex-1 mb-6">
                      <div className="h-28 bg-muted/40 rounded-2xl p-4 border border-border overflow-hidden relative group/code hover:border-primary/20 transition-all">
                         <div className="text-[11px] font-mono leading-relaxed text-foreground/50 italic">
                            <HighlightedText text={prompt.content} />
                         </div>
                         <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-muted to-transparent group-hover/code:opacity-40 transition-opacity" />
                         ...
                         {prompt.projectIds && prompt.projectIds.length > 0 && (
                            <div className="absolute top-2 right-2 flex gap-1">
                                {prompt.projectIds.map(pid => {
                                    ...
                                    if (!p) return null;
                                    return (
                                        <div key={pid} className="w-2.5 h-2.5 rounded-full border border-border shadow-sm" style={{ backgroundColor: p.color }} title={p.name} />
                                    );
                                })}
                            </div>
                         )}
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-border">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {prompt.tags.slice(0, 3).map(tag => (
                            <div key={tag} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-muted border border-border text-[9px] font-black uppercase tracking-widest text-foreground/40">
        ...
        <Dialog open={formOpen} onOpenChange={setFormOpen}>
          <DialogContent className="glass border-border max-w-2xl rounded-[40px] p-10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30 ml-2">{t.common.title}</label>
                    <Input value={formTitle} onChange={e => setFormTitle(e.target.value)} className="glass h-12 px-5 rounded-2xl border-border font-bold italic" placeholder="Creative Refactoring..." />
                </div>
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30 ml-2">{t.common.category}</label>
                    <select
                        value={formCategory}
                        onChange={e => setFormCategory(e.target.value)}
                        className="w-full h-12 rounded-2xl glass border-border px-4 text-xs font-bold uppercase tracking-widest focus:outline-none focus:ring-1 focus:ring-primary/40 appearance-none bg-background"
                    >
                        {CATEGORIES.map(c => (
                            <option key={c} value={c} className="bg-background border-none">{c}</option>
                        ))}
                    </select>
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30 ml-2">{t.common.description}</label>
                <Input value={formDesc} onChange={e => setFormDesc(e.target.value)} className="glass h-12 px-5 rounded-2xl border-border font-medium italic text-foreground/60" placeholder="Used for optimizing complex logic branches..." />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30 ml-2">{t.common.tags}</label>
                <Input value={formTags} onChange={e => setFormTags(e.target.value)} className="glass h-12 px-5 rounded-2xl border-border font-mono text-[10px] text-primary" placeholder="coding, rust, cleanup" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30 ml-2">Linked Projects</label>
                <div className="flex flex-wrap gap-2 p-3 glass rounded-2xl border-border min-h-[50px]">
                    {projects.map((p: any) => (
                        <button
                            key={p.id}
                            onClick={() => {
                            ...
                            className={cn(
                                ...
                                formProjectIds.includes(p.id) 
                                    ...
                                    : "bg-muted/50 border-border text-foreground/40 hover:bg-muted"
                            )}
                        >
                    ...
                    {projects.length === 0 && <span className="text-[10px] text-foreground/20 italic p-1">No projects available to link.</span>}
                </div>
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between ml-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-foreground/30">{t.prompts.promptContent}</label>
                ...
                <Textarea
                  ...
                  className="min-h-[250px] glass p-6 rounded-[28px] border-border font-mono text-[13px] leading-relaxed italic resize-none focus:border-primary/20"
                />
              </div>
            </div>
            <DialogFooter className="mt-10 gap-3">
              <Button variant="ghost" onClick={() => setFormOpen(false)} className="h-14 px-8 rounded-full text-foreground/40 hover:text-foreground uppercase font-black tracking-widest text-[10px]">{t.common.cancel}</Button>
        ...
        <Dialog open={!!deleteDialogId} onOpenChange={() => setDeleteDialogId(null)}>
          <DialogContent className="glass border-border max-w-sm rounded-[40px] p-10">
            <DialogHeader className="mb-6">
                <DialogTitle className="text-2xl font-black uppercase italic tracking-tighter flex items-center gap-3">
                    <Trash2 className="w-6 h-6 text-destructive" /> {t.prompts.deletePrompt}
                </DialogTitle>
            </DialogHeader>
            <p className="text-foreground/60 font-medium italic mb-10 leading-relaxed text-center">{t.prompts.deletePromptConfirm}</p>
            <DialogFooter className="gap-3">
                <Button variant="ghost" onClick={() => setDeleteDialogId(null)} className="flex-1 h-12 rounded-2xl text-foreground/40 hover:text-foreground uppercase font-black text-[10px] tracking-widest">{t.common.cancel}</Button>
